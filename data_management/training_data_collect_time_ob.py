...
def evaluate_directory(subdir):
    ...
    with open(os.path.join(subdir, f"cost_matrix_{k}_330.json"), 'w') as f_costs:
...
def evaluate_all_mutil_process(data_directory):
    ...
    for subdir, _, files in os.walk(data_directory):
        if f"training_data_{k}.json" in files and "meta_data.json" in files and "parameter_new.json" in files:
...
if __name__ == "__main__":
    ...
    # evaluate_directory("../training_data/JOB_3300/job_1")
